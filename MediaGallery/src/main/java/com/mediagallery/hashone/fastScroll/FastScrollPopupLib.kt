...
class FastScrollPopupLib(private val mRes: Resources, private val mFastRecyclerLib: ScrollFastRecyclerLib) {
    ...
    var alpha = 1f
        ...
        set(alpha) {
            field = alpha
            mFastRecyclerLib.invalidate(mBgBounds)
        }
    ...
    private fun createRadii(): FloatArray {
        if (popupPosition == FastScrollerLib.FastScrollerPopupPosition.CENTER) {
    ...
    fun updateFastScrollerBounds(fastRecyclerLib: ScrollFastRecyclerLib, thumbOffsetY: Int): Rect {
        mInvalidateRect.set(mBgBounds)

        if (isVisible) {
            val edgePadding = fastRecyclerLib.scrollBarWidth
            ...
            if (popupPosition == FastScrollerLib.FastScrollerPopupPosition.CENTER) {
                mBgBounds.left = (fastRecyclerLib.width - bgWidth) / 2
                mBgBounds.right = mBgBounds.left + bgWidth
                mBgBounds.top = (fastRecyclerLib.height - bgHeight) / 2
            } else {
                if (Utils.isRtl(mRes)) {
                    mBgBounds.left = 2 * fastRecyclerLib.scrollBarWidth
                    mBgBounds.right = mBgBounds.left + bgWidth
                } else {
                    mBgBounds.right = fastRecyclerLib.width - 2 * fastRecyclerLib.scrollBarWidth
                    mBgBounds.left = mBgBounds.right - bgWidth
                }
                mBgBounds.top = thumbOffsetY - bgHeight + fastRecyclerLib.scrollBarThumbHeight / 2
                mBgBounds.top = Math.max(edgePadding, Math.min(mBgBounds.top, fastRecyclerLib.height - edgePadding - bgHeight))
            }
