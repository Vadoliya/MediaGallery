...
class FastScrollerLib(context: Context, private val mFastRecyclerLib: ScrollFastRecyclerLib?, attrs: AttributeSet) {
    ...
    private val mPopupLib: FastScrollPopupLib
    val thumbHeight: Int
    val width: Int
    ...
    init {
        ...
        mPopupLib = FastScrollPopupLib(resources, mFastRecyclerLib!!)
        ...
        try {
            ...
            mPopupLib.setBgColor(popupBgColor)
            mPopupLib.setTextColor(popupTextColor)
            mPopupLib.setTextSize(popupTextSize)
            mPopupLib.setBackgroundSize(popupBackgroundSize)
            mPopupLib.popupPosition = popupPosition
        } finally {
            typedArray.recycle()
        }
        ...
        mHideRunnable = Runnable {
            if (!isDragging) {
                ...
                mAutoHideAnimator = ObjectAnimator.ofInt(this@FastScrollerLib, "offsetX", (if (Utils.isRtl(
                        mFastRecyclerLib.resources
                    )
                ) -1 else 1) * width)
        ...
        mFastRecyclerLib.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                super.onScrolled(recyclerView, dx, dy)

                if (!mFastRecyclerLib.isInEditMode) {
                    show()
                }
            }
        })
    ...
    fun handleTouchEvent(ev: MotionEvent, downX: Int, downY: Int, lastY: Int,
        ...
        val config = ViewConfiguration.get(mFastRecyclerLib!!.context)
        ...
        when (action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                if (!isDragging && isNearPoint(downX, downY) &&
                    ...
                    mFastRecyclerLib.parent.requestDisallowInterceptTouchEvent(true)
                    isDragging = true
                    mTouchOffset += lastY - downY
                    mPopupLib.animateVisibility(true)
                ...
                if (isDragging) {
                    val top = 0
                    val bottom = mFastRecyclerLib.height - thumbHeight
                    val boundedY = Math.max(top, Math.min(bottom, y - mTouchOffset)).toFloat()
                    val sectionName = mFastRecyclerLib.scrollToPositionAtProgress((boundedY - top) / (bottom - top))
                    mPopupLib.setSectionName(sectionName)
                    mPopupLib.animateVisibility(!sectionName.isEmpty())
                    mFastRecyclerLib.invalidate(mPopupLib.updateFastScrollerBounds(mFastRecyclerLib, mThumbPosition.y))
                }
            }
            MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                ...
                if (isDragging) {
                    isDragging = false
                    mPopupLib.animateVisibility(false)
    ...
    fun draw(canvas: Canvas) {
        ...
        canvas.drawRect((mThumbPosition.x + mOffset.x).toFloat(), mOffset.y.toFloat(), (mThumbPosition.x + mOffset.x + width).toFloat(), (mFastRecyclerLib!!.height + mOffset.y).toFloat(), mTrack)
    ...
    fun setThumbPosition(x: Int, y: Int) {
        ...
        mInvalidateRect.set(mThumbPosition.x + mOffset.x, mOffset.y, mThumbPosition.x + mOffset.x + width, mFastRecyclerLib!!.height + mOffset.y)
        mThumbPosition.set(x, y)
        mInvalidateTmpRect.set(mThumbPosition.x + mOffset.x, mOffset.y, mThumbPosition.x + mOffset.x + width, mFastRecyclerLib.height + mOffset.y)
        mInvalidateRect.union(mInvalidateTmpRect)
        mFastRecyclerLib.invalidate(mInvalidateRect)
    }
    ...
    private fun postAutoHideDelayed() {
        if (mFastRecyclerLib != null) {
            cancelAutoHide()
            mFastRecyclerLib.postDelayed(mHideRunnable, mAutoHideDelay.toLong())
        }
    }

    private fun cancelAutoHide() {
        mFastRecyclerLib?.removeCallbacks(mHideRunnable)
    }
    ...
    fun setPopupBgColor(@ColorInt color: Int) {
        mPopupLib.setBgColor(color)
    }

    fun setPopupTextColor(@ColorInt color: Int) {
        mPopupLib.setTextColor(color)
    }

    fun setPopupTypeface(typeface: Typeface) {
        mPopupLib.setTypeface(typeface)
    }

    fun setPopupTextSize(size: Int) {
        mPopupLib.setTextSize(size)
    }
    ...
    fun setPopupTextTypeface(typeface: Typeface){
        mPopupLib.mPopupTextTypeface = typeface
    }
